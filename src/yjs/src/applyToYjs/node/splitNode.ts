import { Node, type SplitNodeOperation, Text } from "slate";
...
import { getStoredPositionsInDeltaAbsolute, restoreStoredPositionsWithDeltaAbsolute } from "../../utils/position";
...
export function splitNode(sharedRoot: Y.XmlText, slateRoot: Node, op: SplitNodeOperation): void {
  ...
  if (!target.yTarget) {
    ...
    return target.yParent.format(target.textRange.start, target.textRange.end - target.textRange.start, {
      ...unset,
      ...op.properties,
    });
  }
  ...
  const ySplitOffset = (target.slateTarget as any).children
    .slice(0, op.position)
