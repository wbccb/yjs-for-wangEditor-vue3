import { type BasePoint, type BaseRange, Element, Node, Text } from "slate";
import * as Y from "yjs";

import type { InsertDelta, RelativeRange, TextRange } from "../module/custom-types";
...
function getAbsolutePositionsInYText(
  ...
  insertDelta.forEach(({ insert }, i) => {
    if (insert instanceof Y.XmlText) {
      Object.assign(
        positions,
        getAbsolutePositionsInYText(absolutePositions, insert, parentPath ? `${parentPath}.${i}` : i.toString()),
      );
    }
  });
...
export function slateRangeToRelativeRange(sharedRoot: Y.XmlText, slateRoot: Node, range: BaseRange): RelativeRange {
  return {
